...
const startMarker = "// Single Proposal Section Regeneration Route";
...
// Find where the second regenerate-proposal-section endpoint ends
const endMarker = "app.post('/api/regenerate-proposal-section', async (req, res) => {";
const firstRouteIdx = content.indexOf(endMarker);
const secondRouteIdx = content.indexOf(endMarker, firstRouteIdx + 1);

if (secondRouteIdx === -1) {
  console.error("Could not find second route idx");
  process.exit(1);
}

// Find end of second route (up to next app.post or end of try/catch block)
const routeEndIdx = content.indexOf("});", secondRouteIdx + 100);

console.log("Found replacement bounds:", { startIndex, firstRouteIdx, secondRouteIdx, routeEndIdx });

const newCodeBlock = `// 2.4 Dynamic Proposal Fallback Generator
function generateDynamicProposalFallback(params: any) {
  ...
  return {
    ...
    literatureReviewText: literatureReview || (isAr
    ...
    researchGapText: researchGap || (isAr
    ...
    methodologyChapterText: typeof methodology === 'string' && methodology.trim() ? methodology : (isAr
...
const updatedContent = content.substring(0, startIndex) + newCodeBlock + content.substring(routeEndIdx + 3);
