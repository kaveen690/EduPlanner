    ...
    <div dir={rtl ? 'rtl' : 'ltr'} className="max-w-7xl mx-auto p-3 sm:p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        >
          ...
          <span>{t('personalLibrary', lang)} ({savedReferences.length})</span>
        </button>
      </div>
      ...
      <div className="relative">
        <form onSubmit={handleSearch} className="relative flex items-center">
          <input
            ...
            className={`w-full ${
              rtl ? 'pr-12 pl-44 text-right' : 'pl-12 pr-44 text-left'
            } py-4 rounded-3xl bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 text-sm font-semibold focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 shadow-md transition-all`}
          />
          <Search className={`w-5 h-5 absolute ${rtl ? 'right-4' : 'left-4'} text-slate-400 pointer-events-none`} />
          ...
          <button
            ...
            className={`absolute ${
              rtl ? 'left-3' : 'right-3'
            } px-5 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-md flex items-center gap-2 transition-all disabled:opacity-50 shrink-0`}
          >
            {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
            <span>{t('searchPapersBtn', lang)}</span>
          </button>
        </form>
      ...
      <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-2">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
            <Filter className="w-3.5 h-3.5 text-blue-500" /> {t('academicSearchFilters', lang)}
          </span>
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 cursor-pointer">
              ...
              />
              <span>{t('openAccessPdfOnly', lang)}</span>
            </label>
            ...
            >
              <option value="relevance">{t('sortByRelevance', lang)}</option>
              <option value="year_desc">{t('sortByNewest', lang)}</option>
              <option value="citations_desc">{t('sortByMostCited', lang)}</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          {/* Year Range */}
          <div>
            <label className="block text-[11px] font-bold text-slate-500 mb-1">{t('yearRange', lang)}</label>
          ...
          {/* Author Filter */}
          <div>
            <label className="block text-[11px] font-bold text-slate-500 mb-1">{t('authorNameLabel', lang)}</label>
            <input
              type="text"
              value={authorFilter}
          ...
          {/* Journal Filter */}
          <div>
            <label className="block text-[11px] font-bold text-slate-500 mb-1">{t('journalPublisherLabel', lang)}</label>
            <select
              value={journalFilter}
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 mb-1">{t('subjectFieldLabel', lang)}</label>
            <select
              value={subjectFilter}
                ...
                >
                  ...
                  <div className="text-xs leading-relaxed text-slate-700 dark:text-slate-300 font-serif">
                    ...
                    >
                      ...
                      {isExpanded ? t('showLessBtn', lang) : t('readFullAbstractBtn', lang)}
                    </button>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-100 dark:border-slate-800 text-xs">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        <ExternalLink className="w-3.5 h-3.5 text-orange-500" /> {t('viewPaperBtn', lang)} ↗
                      </a>

                      {/* Download PDF / Open Access PDF Search */}
                      {(() => {
                        const activePdfUrl = item.pdfUrl || `https://scholar.google.com/scholar?q=filetype:pdf+${encodeURIComponent(item.title)}`;
                        return (
                          <a
                            href={activePdfUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={e => e.stopPropagation()}
                            className="px-2.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-1 transition-all text-xs shadow-xs"
                            title="داگرتنا فایلا PDF / Open Access PDF Search"
                          >
                            <Download className="w-3.5 h-3.5" /> {lang === 'bad' ? 'داگرتنا فایلا PDF' : lang === 'ku' ? 'داگرتنی فایلی PDF' : lang === 'ar' ? 'تحميل ملف PDF' : 'Download PDF'}
                          </a>
                        );
                      })()}
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5 text-purple-500" /> {t('summarizeBtn', lang)}
                      </button>
                      ...
                      >
                        ...
                        {copiedId === item.id ? t('copied', lang) : t('citeBtn', lang)}
                      </button>
                    </div>
